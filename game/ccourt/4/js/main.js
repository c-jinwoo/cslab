...
const img_Arr	= [	
	...
	["main/mobile/arrow.png"],
	["main/mobile/power.png"],																	// 45
	["main/mobile/gage_outer.png"],	
	["main/mobile/gage_inner.png"],	
];
const aud_Arr	= [
	["bgm.mp3"],																				// 0
	["btn_click.mp3"],
...
const Mobile_arrow = function(){
	this.x = 140;this.y = 285;this.w = 143;this.h = 72;
	this.cx = 0;this.cy = 0;this.cw = 143;this.ch = 72;
	this.rot = 0;
	this.isDown = false;
	this.active = false;
	this.draw = function(){
		try{
			if(this.active){
				ctx.save();
				ctx.translate(this.x * wR, (this.y + this.h * 0.5) * hR);
				ctx.rotate(this.rot);
				ctx.translate(-1 * this.x * wR, -1 * (this.y +  this.h * 0.5) * hR);
...
const Mobile_power = function(){
	this.x = 340;this.y = 400;this.w = 119;this.h = 120;
	this.cx = 0;this.cy = 0;this.cw = 119;this.ch = 120;
	this.active = false;
	this.draw = function(){
		try{
			if(this.active){	
...
const Mobile_gage_outer = function(){
	this.x = 275;this.y = 350;this.w = 251;this.h = 49;
	this.cx = 0;this.cy = 0;this.cw = 251;this.ch = 49;
	this.active = false;
	this.draw = function(){
		try{
			if(this.active){	
...
const Mobile_gage_inner = function(){
	this.x = 278;this.y = 365;this.w = 0;this.h = 26;
	this.cx = 0;this.cy = 0;this.cw = 0;this.ch = 26;
	this.oriW = 244;
	this.isDown = false;
	this.active = false;
	this.draw = function(){
...
let mobile_arrow = new Object(),
	mobile_power = new Object(),
	mobile_gage_outer = new Object(),
	mobile_gage_inner = new Object();
let world = new Object(),
	ball = new Object(),
	wood = new Object(),
	basket_front = new Object(),
	basket_back = new Object(),	
	obj_Arr = new Array(),
	mission_arr = new Array(1, 1, 1, 2, 3);
/* 연동 변수 */
let loggedIn = true;
let userId = "id";
let _isPorted = false;
...
function main(){
	...
	(function(){
		...
		mobile_arrow = new Mobile_arrow();
		mobile_power = new Mobile_power();
		mobile_gage_outer = new Mobile_gage_outer();
		mobile_gage_inner = new Mobile_gage_inner();
	...
	function boxObjExplode(_this, _thisW, _thisH){
		...
		obj_Arr[3][obj_Arr[3].length-1].w = _thisW * 30;
		obj_Arr[3][obj_Arr[3].length-1].h = _thisH * 30;
		_this.destroy();
	}
	...
	function draw(){
		...
		if(device != "PC"){
			mobile_arrow.draw();
			mobile_power.draw();
			mobile_gage_outer.draw();
			mobile_gage_inner.draw();
		}
		/* 막 */
...
function boxLineFunc(){
	...
	if(this._canvasPointerInfos){
		...
		let _x2 = 150;
		let _y = 320;
		let basket_front_x = ((basket_front.position().x + basket_front.imageOffsetX()) * 30 + 16);
		let basket_front_y = ((basket_front.position().y + basket_front.imageOffsetY()) * 30 - 9);
		let basket_back_x = ((basket_back.position().x + basket_back.imageOffsetX()) * 30 + 22.5);
		let basket_back_y = ((basket_back.position().y + basket_back.imageOffsetY()) * 30 - 5);
		ctx.lineWidth = 1;ctx.strokeStyle = "#665B36";			
		ctx.beginPath();
		ctx.moveTo(_x1 * wR, _y * hR);
		ctx.lineTo(basket_front_x * wR, basket_front_y * hR);
		ctx.stroke();
		ctx.beginPath();
		ctx.moveTo(_x2 * wR, _y * hR);
		ctx.lineTo(basket_back_x * wR, basket_back_y * hR);
		ctx.stroke();
	}
}
function mobileBtnCtrl(){
	mobile_arrow.reset();
	mobile_power.reset();
	mobile_gage_outer.reset();
	mobile_gage_inner.reset();
}
/* 이벤트관련 */
...
document.addEventListener(mouseEv("move"),function(e){
	...
	if(pageType == "intro"){
		...
		if(!popup){
			if(!mobile_arrow.isDown && !mobile_gage_inner.isDown){
				if(mouseCd(exit_btn)){
					$("canvas").css("cursor","pointer");
			...
			if(device != "PC"){
				if(mobile_arrow.active){
					if(mobile_arrow.isDown){
						let x_axis= mouseX / wR - mobile_arrow.x;
						let y_axis= mouseY / wR - mobile_arrow.y;

						mobile_arrow.rot = Math.atan2(y_axis,x_axis);
					}		
				}
			}
		}
...
document.addEventListener(mouseEv("down"),function(e){ 
	...
	if(pageType == "main"){
		if(!popup){
			if(device != "PC"){
				if(mobile_arrow.active){
					if(mouseCd(mobile_arrow)){	
						mobile_arrow.isDown = true;		
					}
					if(mouseCd(mobile_power)){	
						mobile_gage_inner.isDown = true;	
					}
				}
			}
		}
	...
	if(pageType == "intro"){
		...
		if(!popup){
			...
			if(device != "PC"){
				if(mobile_arrow.active){
					if(mobile_arrow.isDown){
						mobile_arrow.isDown = false;
					}
					if(mobile_gage_inner.isDown){	
						mobile_arrow.active = false;
						mobile_power.active = false;
						mobile_gage_outer.active = false;
						mobile_gage_inner.active = false;
						mobile_gage_inner.isDown = false;
						...
						ball.applyImpulse(80 * mobile_gage_inner.w / mobile_gage_inner.oriW, mobile_arrow.rot * 180 / Math.PI + 90);
						setTimeout(function(){
							if(mission_num >= mission_arr[stage - 1]){
								if(stage < 5){
									successPopupFunc(true);
...
function cvsSet(){
	...
	const widthBase = function(){
		...
		if(pageType == "main"){
			world.scale(30 * cvs.width / absWidth);
		}
	};
